...
public class FileUtil {
  ...
  public static String toDownZip (List<String> urls, String zipName)throws Exception {
    return toDownZip(urls, zipName,"zipFolder");
  }


  public static String toDownZip (List<String> urls, String zipName,String folderName) throws Exception {
    String zipFilePath =  System.getProperties().getProperty("user.home")+ "\\Libra" + "\\" + folderName;
    String zipPath =  System.getProperties().getProperty("user.home")+ "\\Libra" + "\\Zip";
    FileUtil.makeDirectory(zipFilePath+"/ddd");
   FileUtil.makeDirectory(zipPath+"/ddd");
    for (String url:urls) {
      try {
        CopyFile(url,zipFilePath+"\\"+FileUtil.getFileName(url));
      }catch (Exception e) { throw new Exception("文件拷贝失败");}
    }
    toZip(zipFilePath,zipPath+"\\"+zipName+"_"+StringUtil.get26id()+".zip",true);
    Boolean is =  deleteDirectory(zipFilePath);
    return zipPath+"\\"+zipName+ StringUtil.get26id();
  }
  /**
   * 压缩成ZIP 方法1
   *
